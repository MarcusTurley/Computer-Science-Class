...
import static Set_3.Entity.MoveDirection.*;
	...
	public static void SearchPath(Macro p_macro) throws InterruptedException {
		...
		do {
			...
			while (!Arrays.equals(m_path.Pos(), m_to.Pos()) && _duplicateCount < 2) {
				...
				if(m_path.AdjacentMoves().size() >= 2)
					p_forks.add(new int[]{ m_path.X(), m_path.Y(), 1 });
...
class ScoreBoard {
	...
	public void RemoveBestMacro(Macro p_macro) {
		MacrosMinMoves.remove(p_macro); m_moveCounts.remove(p_macro.RecordingID());
		MacrosMinTimes.remove(p_macro); m_moveTimes.remove(p_macro.RecordingID());
	}
}

// Class that stores the mouse position
class Entity {
	public enum MoveDirection { Forward, Right, Back, Left, rUp, rDown, rRight, rLeft;
	...
	public void Move(MoveDirection p_dir, int p_distance) {
		if(p_dir.toInt() < rUp.toInt()) {
			m_moveDirection = p_dir; m_moveDistance = p_distance;
			if (CanMove()) {
	...
	public boolean CanMove(MoveDirection p_dir, int p_distance) {
		...
		int _moveX = (m_move[0] + m_x); int _moveY = (m_move[1] + m_y);
		int _sectionWidth = m_gameBoard.Width() / (m_gameBoard.Columns() + 1);
		int _sectionHeight = m_gameBoard.Height() / (m_gameBoard.Rows() + 1);
		int _minX = _sectionWidth * (_moveX / _sectionWidth); int _maxX = _minX + _sectionWidth;
		int _minY = _sectionHeight * (_moveY / _sectionHeight); int _maxY = _minY + _sectionHeight;
		
		boolean _canMove = true;
		_canMove &= _moveX > _minX && _moveX < _maxX;
		_canMove &= _moveY > _minY && _moveY < _maxY;
	...
	public List<MoveDirection> AdjacentMoves(MoveDirection[] p_ignores) {
		...
		for(int i = 0; i < Math.min(4, _moves.size()); i++)
			if(_moves.contains(MoveDirection.toEnum(i)) && _moves.contains(MoveDirection.toEnum(i + rUp.toInt())))
				ArrayUtilities.ArraySwap(_moves.toArray(), _moves.indexOf(MoveDirection.toEnum(i)), _moves.indexOf(MoveDirection.toEnum(i + rUp.toInt())));
...
class GameBoard {
	...
	public void InitializeBoards(String[] p_arr, int p_rows, int p_columns, int p_margins, int p_border) {
		...
		m_gameMargins = p_margins; m_gameBorder = p_border;
		
		m_gameWidth = p_arr[0].length(); m_displayWidth = p_arr[0].length() + (m_gameColumns * m_gameBorder);
		m_gameHeight = p_arr.length; m_displayHeight = p_arr.length + (m_gameRows * m_gameBorder);
		
		m_gameMin = m_gameBorder + m_gameMargins;
		m_gameMaxX = m_gameMin + m_gameWidth; m_gameMaxY = m_gameMin + m_gameHeight;
		m_displayMaxX = m_gameMin + m_displayWidth; m_displayMaxY = m_gameMin + m_displayHeight;
		...
		for (int i = Math.max(p_y1, 0); i < Math.min(p_y2 + 1, p_arr.length); i++)
			for (int j = Math.max(p_x1, 0); j < Math.min(p_x2 + 1, p_arr[i].length); j++) {
				if(Objects.equals(p_arr[i][j], p_value)) {
					_index[0] = j; _index[1] = i; return _index;
				}
			}
		return _index;
	}
