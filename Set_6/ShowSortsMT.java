...
import static Set_6.ShowSortsMT.Game.Data.*;

public class ShowSortsMT {
	public static void main(String[] args) {
		...
		do {
			...
			do {
				...
				ShowSortsMT.User.UserChar();
			} while (ShowSortsMT.User.UserInput().charAt(0) != 'Y' && ShowSortsMT.User.UserInput().charAt(0) != 'N');
		} while (ShowSortsMT.User.UserInput().charAt(0) != 'N');
	}
	...
	private static void ProblemOne() {
		...
		ShowSortsMT.Game.Data.DisplayResults();
	...
	private static void Solve() {
		for (int ___case = 1; ___case <= ShowSortsMT.FileData.TestCasesCount(); ___case++) {
			String __line;
			String[] __lineArr;
			String __wordList = "", __sortType = "", __sortOrder = "";
			boolean _validInput = false;
			
			do {
				System.out.print("Enter word list: ");
				__line = ShowSortsMT.User.UserString();
				if(__line.isEmpty()) System.out.println("\nYou entered nothing.\n Please try again");
			} while (__line.isEmpty());
			
			__wordList = __line;
			
			do {
				System.out.print("Bubble Sort [B], Selection Sort [S], or Insert Sort [I]: ");
				__line = User.UserString();
				if(__line.isEmpty()) System.out.println("You entered nothing.\n Please try again\n");
				else {
					String[] ____sortTypes = { "b", "s", "i"};
					__sortType = __line.toLowerCase().charAt(0) + "";
					if(Arrays.stream(____sortTypes).anyMatch(__sortType::equalsIgnoreCase))
							_validInput = true;
					if(!_validInput) System.out.println("You did not enter a valid sort type.\n Please try again\n");
				}
			} while (__line.isEmpty() || !_validInput);
			...
			do {
				...
				if(__line.isEmpty()) System.out.println("You entered nothing.\n Please try again\n");
					...
					String[] ____sortOrders = { "a", "d"};
					__sortOrder = __line.toLowerCase().charAt(0) + "";
					if(Arrays.stream(____sortOrders).anyMatch(__sortOrder::equalsIgnoreCase))
						_validInput = true;
					if(!_validInput) System.out.println("You did not enter a valid sort type.\n Please try again\n");
				}
			} while (__line.isEmpty() || !_validInput);
			...
			System.out.println("Original: " + __wordList);
			...
			switch (__sortOrder) {
				case "a" -> {
					__order = 1;
					__orderText = "Ascending";
				}
				case "d" -> {
					__order = 2;
					__orderText = "Descending";
				}
			}
			
			
			switch (__sortType) {
				case "b" -> System.out.println("Bubble Sort (" + __orderText + ") traversal: " + Algorithm.BubbleSort(__wordList, __order));
				case "s" -> System.out.println("Selection Sort (" + __orderText + ") traversal: " + Algorithm.SelectionSort(__wordList, __order));
				case "i" -> System.out.println("Insert Sort (" + __orderText + ") traversal: " + Algorithm.InsertSort(__wordList, __order));
			}
	...
	private static class Algorithm {
		private static String BubbleSort(String p_str, int _order) {
			String _result = "";
			String[] _resultArr = p_str.toLowerCase().split(" ");
			
			return _result = p_str;
		}
		private static String SelectionSort(String p_str, int _order) {
			String _result = "";
			String[] _resultArr = p_str.toLowerCase().split(" ");
			
			return _result = p_str;
		}
		private static String InsertSort(String p_str, int _order) {
			String _result = "";
			String[] _resultArr = p_str.toLowerCase().split(" ");
	...
	protected static class Game {
		...
		public enum Direction {
			...
			public static ShowSortsMT.Game.Direction toEnum(int p_val) {
			...
			public ShowSortsMT.Game.Direction opposite() {
				switch (this) {
		...
		protected static class Data {
			...
			private static ShowSortsMT.Game.Data.GameState m_state = ShowSortsMT.Game.Data.GameState.Initializing;
			private static ShowSortsMT.Game.Data.GameMode m_mode = ShowSortsMT.Game.Data.GameMode.None;
			private static ShowSortsMT.Game.Data.GameResult m_result = ShowSortsMT.Game.Data.GameResult.None;
			
			public static ShowSortsMT.Game.Data.GameState GameState() { return m_state; }
			public static ShowSortsMT.Game.Data.GameMode GameMode() { return m_mode; }
			public static ShowSortsMT.Game.Data.GameResult GameResult() { return m_result; }
			
			public static void SetGameState(ShowSortsMT.Game.Data.GameState p_state) { m_state = p_state; }
			public static void SetGameMode(ShowSortsMT.Game.Data.GameMode p_mode) { m_mode = p_mode; }
			public static void SetGameResult(ShowSortsMT.Game.Data.GameResult p_result) { m_result = p_result; }
			
			public static void Init() { Init(ShowSortsMT.Game.Data.GameMode.None); }
			public static void Init(ShowSortsMT.Game.Data.GameMode p_mode) {
				SetGameState(ShowSortsMT.Game.Data.GameState.Initializing);
				SetGameMode(p_mode);
			}
			...
			public static void GetGameMode() {
				do {
					...
					ShowSortsMT.User.UserChar();
					if (!ShowSortsMT.User.IsLineEmpty()) {
						switch (Character.toUpperCase(ShowSortsMT.User.UserInput().charAt(0))) {
							case 'G':
								SetGameMode(ShowSortsMT.Game.Data.GameMode.Game);
								SetGameState(ShowSortsMT.Game.Data.GameState.Playing);
								break;
							case 'D':
								SetGameMode(ShowSortsMT.Game.Data.GameMode.Demo);
								SetGameState(ShowSortsMT.Game.Data.GameState.Playing);
								break;
						}
					}
				} while (GameMode() == ShowSortsMT.Game.Data.GameMode.None);
			}
			
			private static void DisplayResults() {
				switch (ShowSortsMT.Game.Data.m_result) {
		...
		private static class Board {
			...
			private static ShowSortsMT.Game.Board ActiveBoard;
			
			public static ShowSortsMT.Game.Board ActiveBoard() { return ActiveBoard; }
			
			
			private static List<ShowSortsMT.Game.Board> GameBoards = new ArrayList<>();
			
			public static List<ShowSortsMT.Game.Board> GameBoards() { return GameBoards; }
			...
			public void PlayBoardRemoveIndex(String p_str) {
				int[] _index = ShowSortsMT.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
			...
			public void PlayBoardSwapIndex(String p_str, int p_x, int p_y) {
				int[] _index = ShowSortsMT.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
				if (_index[0] > -1 && _index[1] > -1) ShowSortsMT.Utils.ArrayU.Array2DSwap(m_playBoard, _index[0], _index[1], p_x, p_y);
			}
			...
			public void RenderBoard(boolean p_updateBoard) {
				...
				for (String[] _arr : m_displayBoard)
					System.out.println(ShowSortsMT.Utils.ArrayU.ArrayToString(_arr, ShowSortsMT.Utils.ArrayU.LongestStringIn2DArray(m_displayBoard)));
			}
		...
		public static void GetUserFileData() {
			...
			while (!m_beenRead) {
				try {
					...
					// Gets user file name
					m_fileName = ShowSortsMT.User.UserString();
					System.out.println();
					...
					try {
						...
						m_beenRead = true;
						_scanner.close();
						if (ShowSortsMT.Utils.StringU.isStringNumber(m_fileData.get(0))) {
	...
	protected static class User {
		...
		public ShowSortsMT.User WaitForValid(String [] p_ignores) {
			return this;
		}
		...
		public static float UserNum() {
			...
			if (ShowSortsMT.Utils.StringU.isStringNumber(m_userInput)) return Integer.parseInt(m_userInput);
			return -1;
		}
	...
	protected static class Utils {
		...
		private static class ArrayU {
			...
			public static <t_Any> String ArrayToString(t_Any[] p_arr, int p_spacing) {
				...
				return ShowSortsMT.Utils.StringU.SpaceEvenly(_result.substring(0, _result.length() - 1), " ", p_spacing);
			}
		...
		private static class StringU {
			...
			public static String SpaceEvenly(String p_str, String p_split, int p_spacing) {
				...
				if (p_spacing < 0) p_spacing = ShowSortsMT.Utils.ArrayU.LongestStringInArray(_items.toArray(new String[0]));
