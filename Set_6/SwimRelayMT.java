...
import static Set_6.SwimRelayMT.Game.Data.*;

public class SwimRelayMT {
	public static void main(String[] args) {
		...
		do {
			...
			do {
				...
				SwimRelayMT.User.UserChar();
			} while (SwimRelayMT.User.UserInput().charAt(0) != 'Y' && SwimRelayMT.User.UserInput().charAt(0) != 'N');
		} while (SwimRelayMT.User.UserInput().charAt(0) != 'N');
	}
	...
	private static void ProblemOne() {
		...
		SwimRelayMT.Game.Data.DisplayResults();
	...
	private static void Solve() {
		...
		for (int _case = 1; _case <= SwimRelayMT.FileData.TestCasesCount(); _case++) {
			List<List<int[]>> __teamTimes = new ArrayList<>();
			while (FileData.Data().size() > FileData.FileIndex()) {
				__teamTimes.add(new ArrayList<>());
				int ___playerCount = Integer.parseInt(FileData.GetCurrentLine());
				for (int ___playerI = 0; ___playerI < ___playerCount; ___playerI++) {
					String[] ___timesStrings = FileData.GetCurrentLine().split(" ");
					int[] ___times = Arrays.stream(___timesStrings).mapToInt(Integer::parseInt).toArray();
					__teamTimes.get(__teamTimes.size() - 1).add(new int[___times.length]);
					for (int ____timeI = 0; ____timeI < ___times.length; ____timeI++) {
						__teamTimes.get(__teamTimes.size() - 1).get(___playerI)[____timeI] = ___times[____timeI];
					}
				}
			}
			
			List<int[]> __bestTimes = new ArrayList<>();
			List<int[]> __bestPlayers = new ArrayList<>();
			for (int _teamI = 0; _teamI < __teamTimes.size(); _teamI++) {
				__bestTimes.add(new int[4]);
				__bestPlayers.add(new int[4]);
				for (int _sportType = 0; _sportType < 4; _sportType++) {
					int ___bestTime = __teamTimes.get(_teamI).get(0)[_sportType];
					int ___bestPlayer = 1;
					for (int ___playerI = 0; ___playerI < __teamTimes.get(_teamI).size(); ___playerI++) {
						if(__teamTimes.get(_teamI).get(___playerI)[_sportType] < ___bestTime) {
							___bestTime = __teamTimes.get(_teamI).get(___playerI)[_sportType];
							___bestPlayer = ___playerI + 1;
						}
						__bestTimes.get(_teamI)[_sportType] = ___bestTime;
						__bestPlayers.get(_teamI)[_sportType] = ___bestPlayer;
					}
				}
			}
			
			String[] __bestPlayersString = new String[__bestPlayers.size()];
			int[] __bestSumTimes = new int[__bestTimes.size()];
			for (int __playerI = 0; __playerI < __bestPlayers.size(); __playerI++) {
				__bestPlayersString[__playerI] = "";
				for (int ___time = 0; ___time < __bestPlayers.get(__playerI).length; ___time++) {
					__bestPlayersString[__playerI] += __bestPlayers.get(__playerI)[___time] + " ";
					__bestSumTimes[__playerI] += __bestTimes.get(__playerI)[___time];
				}
			}
			
			String[] __sumTimesString = new String[__bestTimes.size()];
			for (int __playerI = 0; __playerI < __bestTimes.size(); __playerI++) {
				__sumTimesString[__playerI] = __bestSumTimes[__playerI] / 60 + ":" + __bestSumTimes[__playerI] % 60;
			}
			
			for (int __teamI = 0; __teamI < __teamTimes.size(); __teamI++) {
			System.out.println("Team " + __teamI + "'s best relay team are swimmers " + __bestPlayersString[__teamI] + "with a total time of " + __sumTimesString[__teamI] + ".");
			}
	...
	protected static class Game {
		...
		public enum Direction {
			...
			public static SwimRelayMT.Game.Direction toEnum(int p_val) {
			...
			public SwimRelayMT.Game.Direction opposite() {
				switch (this) {
		...
		protected static class Data {
			...
			private static SwimRelayMT.Game.Data.GameState m_state = SwimRelayMT.Game.Data.GameState.Initializing;
			private static SwimRelayMT.Game.Data.GameMode m_mode = SwimRelayMT.Game.Data.GameMode.None;
			private static SwimRelayMT.Game.Data.GameResult m_result = SwimRelayMT.Game.Data.GameResult.None;
			
			public static SwimRelayMT.Game.Data.GameState GameState() { return m_state; }
			public static SwimRelayMT.Game.Data.GameMode GameMode() { return m_mode; }
			public static SwimRelayMT.Game.Data.GameResult GameResult() { return m_result; }
			
			public static void SetGameState(SwimRelayMT.Game.Data.GameState p_state) { m_state = p_state; }
			public static void SetGameMode(SwimRelayMT.Game.Data.GameMode p_mode) { m_mode = p_mode; }
			public static void SetGameResult(SwimRelayMT.Game.Data.GameResult p_result) { m_result = p_result; }
			
			public static void Init() { Init(SwimRelayMT.Game.Data.GameMode.None); }
			public static void Init(SwimRelayMT.Game.Data.GameMode p_mode) {
				SetGameState(SwimRelayMT.Game.Data.GameState.Initializing);
				SetGameMode(p_mode);
			}
			...
			public static void GetGameMode() {
				do {
					...
					SwimRelayMT.User.UserChar();
					if (!SwimRelayMT.User.IsLineEmpty()) {
						switch (Character.toUpperCase(SwimRelayMT.User.UserInput().charAt(0))) {
							case 'G':
								SetGameMode(SwimRelayMT.Game.Data.GameMode.Game);
								SetGameState(SwimRelayMT.Game.Data.GameState.Playing);
								break;
							case 'D':
								SetGameMode(SwimRelayMT.Game.Data.GameMode.Demo);
								SetGameState(SwimRelayMT.Game.Data.GameState.Playing);
								break;
						}
					}
				} while (GameMode() == SwimRelayMT.Game.Data.GameMode.None);
			}
			
			private static void DisplayResults() {
				switch (SwimRelayMT.Game.Data.m_result) {
		...
		private static class Board {
			...
			private static SwimRelayMT.Game.Board ActiveBoard;
			
			public static SwimRelayMT.Game.Board ActiveBoard() { return ActiveBoard; }
			
			
			private static List<SwimRelayMT.Game.Board> GameBoards = new ArrayList<>();
			
			public static List<SwimRelayMT.Game.Board> GameBoards() { return GameBoards; }
			...
			public void PlayBoardRemoveIndex(String p_str) {
				int[] _index = SwimRelayMT.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
			...
			public void PlayBoardSwapIndex(String p_str, int p_x, int p_y) {
				int[] _index = SwimRelayMT.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
				if (_index[0] > -1 && _index[1] > -1) SwimRelayMT.Utils.ArrayU.Array2DSwap(m_playBoard, _index[0], _index[1], p_x, p_y);
			}
			...
			public void RenderBoard(boolean p_updateBoard) {
				...
				for (String[] _arr : m_displayBoard)
					System.out.println(SwimRelayMT.Utils.ArrayU.ArrayToString(_arr, SwimRelayMT.Utils.ArrayU.LongestStringIn2DArray(m_displayBoard)));
			}
	...
	private static class FileData {
		...
		public static String GetCurrentLine(boolean _increment) {
			if(m_fileData.size() > m_fileIndex) {
				if(_increment) m_fileIndex++;
				return m_fileData.get(m_fileIndex - 1);
			}
			return "";
		};
		...
		public static void GetUserFileData() {
			...
			while (!m_beenRead) {
				try {
					...
					// Gets user file name
					m_fileName = SwimRelayMT.User.UserString();
					System.out.println();
					...
					try {
						...
						m_beenRead = true;
						_scanner.close();
						if (SwimRelayMT.Utils.StringU.isStringNumber(m_fileData.get(0))) {
	...
	private static class User {
		...
		public SwimRelayMT.User WaitForValid(String [] p_ignores) {
			return this;
		}
		...
		public static float UserNum() {
			...
			if (SwimRelayMT.Utils.StringU.isStringNumber(m_userInput)) return Integer.parseInt(m_userInput);
			return -1;
		}
	...
	protected static class Utils {
		...
		private static class ArrayU {
			...
			public static <t_Any> String ArrayToString(t_Any[] p_arr, int p_spacing) {
				...
				return SwimRelayMT.Utils.StringU.SpaceEvenly(_result.substring(0, _result.length() - 1), " ", p_spacing);
			}
		...
		private static class StringU {
			...
			public static String SpaceEvenly(String p_str, String p_split, int p_spacing) {
				...
				if (p_spacing < 0) p_spacing = SwimRelayMT.Utils.ArrayU.LongestStringInArray(_items.toArray(new String[0]));
