...
import static Set_5.FindTraversals.Game.Data.*;

public class FindTraversals {
	public static void main(String[] args) {
		...
		do {
			...
			do {
				...
				FindTraversals.User.UserChar();
			} while (FindTraversals.User.UserInput().charAt(0) != 'Y' && FindTraversals.User.UserInput().charAt(0) != 'N');
		} while (FindTraversals.User.UserInput().charAt(0) != 'N');
	}
	...
	private static void ProblemOne() {
		...
		FindTraversals.Game.Data.DisplayResults();
	...
	private static void Solve() {
		for (int ___case = 1; ___case <= FindTraversals.FileData.TestCases(); ___case++) {
			String __line;
			String[] __lineArr = { "", "" };
			String[] __traversals = { "i", "r", "p"};
			boolean _validInput = false;
			do {
				System.out.println("In-Order[I] Pre-Order[R] Post-Order[P]");
				System.out.print("Enter traversals: ");
				__line = User.UserString();
				if(__line.isEmpty()) System.out.println("You entered nothing.\n Please try again\n");
				else if(__line.length() > 1){
					__lineArr = __line.toLowerCase().split(" ");
					if(Arrays.stream(__traversals).anyMatch(__lineArr[0]::equalsIgnoreCase))
						if(Arrays.stream(__traversals).anyMatch(__lineArr[1]::equalsIgnoreCase))
							_validInput = true;
					if(!_validInput) System.out.println("You did not enter 2 valid traversal types.\n Please try again\n");
				} else System.out.println("You only entered one input.\n Please try again\n");
			} while (__line.isEmpty() || !_validInput);
			SpaceScreen();
			
			int _findTraversal = -1;
			if(Arrays.stream(__lineArr).noneMatch("i"::equalsIgnoreCase))
				_findTraversal = 1;
			else if(Arrays.stream(__lineArr).noneMatch("r"::equalsIgnoreCase))
				_findTraversal = 2;
			else if(Arrays.stream(__lineArr).noneMatch("p"::equalsIgnoreCase))
				_findTraversal = 3;
			
			do {
				System.out.print("Enter sentence: ");
				__line = FindTraversals.User.UserString();;
	...
	protected static class Game {
		...
		public enum Direction {
			...
			public static FindTraversals.Game.Direction toEnum(int p_val) {
			...
			public FindTraversals.Game.Direction opposite() {
				switch (this) {
		...
		protected static class Data {
			...
			private static FindTraversals.Game.Data.GameState m_state = FindTraversals.Game.Data.GameState.Initializing;
			private static FindTraversals.Game.Data.GameMode m_mode = FindTraversals.Game.Data.GameMode.None;
			private static FindTraversals.Game.Data.GameResult m_result = FindTraversals.Game.Data.GameResult.None;
			
			public static FindTraversals.Game.Data.GameState GameState() { return m_state; }
			public static FindTraversals.Game.Data.GameMode GameMode() { return m_mode; }
			public static FindTraversals.Game.Data.GameResult GameResult() { return m_result; }
			
			public static void SetGameState(FindTraversals.Game.Data.GameState p_state) { m_state = p_state; }
			public static void SetGameMode(FindTraversals.Game.Data.GameMode p_mode) { m_mode = p_mode; }
			public static void SetGameResult(FindTraversals.Game.Data.GameResult p_result) { m_result = p_result; }
			
			public static void Init() { Init(FindTraversals.Game.Data.GameMode.None); }
			public static void Init(FindTraversals.Game.Data.GameMode p_mode) {
				SetGameState(FindTraversals.Game.Data.GameState.Initializing);
				SetGameMode(p_mode);
			}
			...
			public static void GetGameMode() {
				do {
					...
					FindTraversals.User.UserChar();
					if (!FindTraversals.User.IsLineEmpty()) {
						switch (Character.toUpperCase(FindTraversals.User.UserInput().charAt(0))) {
							case 'G':
								SetGameMode(FindTraversals.Game.Data.GameMode.Game);
								SetGameState(FindTraversals.Game.Data.GameState.Playing);
								break;
							case 'D':
								SetGameMode(FindTraversals.Game.Data.GameMode.Demo);
								SetGameState(FindTraversals.Game.Data.GameState.Playing);
								break;
						}
					}
				} while (GameMode() == FindTraversals.Game.Data.GameMode.None);
			}
			
			private static void DisplayResults() {
				switch (FindTraversals.Game.Data.m_result) {
		...
		private static class Board {
			...
			private static FindTraversals.Game.Board ActiveBoard;
			
			public static FindTraversals.Game.Board ActiveBoard() { return ActiveBoard; }
			
			
			private static List<FindTraversals.Game.Board> GameBoards = new ArrayList<>();
			
			public static List<FindTraversals.Game.Board> GameBoards() { return GameBoards; }
			...
			public void PlayBoardRemoveIndex(String p_str) {
				int[] _index = FindTraversals.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
			...
			public void PlayBoardSwapIndex(String p_str, int p_x, int p_y) {
				int[] _index = FindTraversals.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
				if (_index[0] > -1 && _index[1] > -1) FindTraversals.Utils.ArrayU.Array2DSwap(m_playBoard, _index[0], _index[1], p_x, p_y);
			}
			...
			public void RenderBoard(boolean p_updateBoard) {
				...
				for (String[] _arr : m_displayBoard)
					System.out.println(FindTraversals.Utils.ArrayU.ArrayToString(_arr, FindTraversals.Utils.ArrayU.LongestStringIn2DArray(m_displayBoard)));
			}
		...
		public static void GetUserFileData() {
			...
			while (!m_beenRead) {
				try {
					...
					// Gets user file name
					m_fileName = FindTraversals.User.UserString();
					System.out.println();
					...
					try {
						...
						m_beenRead = true;
						_scanner.close();
						if (FindTraversals.Utils.StringU.isStringNumber(m_fileData.get(0))) {
	...
	private static class User {
		...
		public FindTraversals.User WaitForValid(String [] p_ignores) {
			return this;
		}
		...
		public static float UserNum() {
			...
			if (FindTraversals.Utils.StringU.isStringNumber(m_userInput)) return Integer.parseInt(m_userInput);
			return -1;
		}
	...
	protected static class Utils {
		...
		private static class ArrayU {
			...
			public static <t_Any> String ArrayToString(t_Any[] p_arr, int p_spacing) {
				...
				return FindTraversals.Utils.StringU.SpaceEvenly(_result.substring(0, _result.length() - 1), " ", p_spacing);
			}
		...
		private static class StringU {
			...
			public static String SpaceEvenly(String p_str, String p_split, int p_spacing) {
				...
				if (p_spacing < 0) p_spacing = FindTraversals.Utils.ArrayU.LongestStringInArray(_items.toArray(new String[0]));
