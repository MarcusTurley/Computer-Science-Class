...
import static Set_5.StringTraversals.Game.Data.*;
...
public class StringTraversals {
	public static void main(String[] args) {
		...
		do {
			...
			do {
				...
				StringTraversals.User.UserChar();
			} while (StringTraversals.User.UserInput().charAt(0) != 'Y' && StringTraversals.User.UserInput().charAt(0) != 'N');
		} while (StringTraversals.User.UserInput().charAt(0) != 'N');
	}
	...
	private static void ProblemOne() {
		...
		StringTraversals.Game.Data.DisplayResults();
	...
	private static void Solve() {
		for (int ___case = 1; ___case <= StringTraversals.FileData.TestCases(); ___case++) {
			String __line;
			do {
	...
	protected static class Game {
		...
		public enum Direction {
			...
			public static StringTraversals.Game.Direction toEnum(int p_val) {
			...
			public StringTraversals.Game.Direction opposite() {
				switch (this) {
		...
		protected static class Data {
			...
			private static StringTraversals.Game.Data.GameState m_state = StringTraversals.Game.Data.GameState.Initializing;
			private static StringTraversals.Game.Data.GameMode m_mode = StringTraversals.Game.Data.GameMode.None;
			private static StringTraversals.Game.Data.GameResult m_result = StringTraversals.Game.Data.GameResult.None;
			
			public static StringTraversals.Game.Data.GameState GameState() { return m_state; }
			public static StringTraversals.Game.Data.GameMode GameMode() { return m_mode; }
			public static StringTraversals.Game.Data.GameResult GameResult() { return m_result; }
			
			public static void SetGameState(StringTraversals.Game.Data.GameState p_state) { m_state = p_state; }
			public static void SetGameMode(StringTraversals.Game.Data.GameMode p_mode) { m_mode = p_mode; }
			public static void SetGameResult(StringTraversals.Game.Data.GameResult p_result) { m_result = p_result; }
			
			public static void Init() { Init(StringTraversals.Game.Data.GameMode.None); }
			public static void Init(StringTraversals.Game.Data.GameMode p_mode) {
				SetGameState(StringTraversals.Game.Data.GameState.Initializing);
				SetGameMode(p_mode);
			}
			...
			public static void GetGameMode() {
				do {
					...
					StringTraversals.User.UserChar();
					if (!StringTraversals.User.IsLineEmpty()) {
						switch (Character.toUpperCase(StringTraversals.User.UserInput().charAt(0))) {
							case 'G':
								SetGameMode(StringTraversals.Game.Data.GameMode.Game);
								SetGameState(StringTraversals.Game.Data.GameState.Playing);
								break;
							case 'D':
								SetGameMode(StringTraversals.Game.Data.GameMode.Demo);
								SetGameState(StringTraversals.Game.Data.GameState.Playing);
								break;
						}
					}
				} while (GameMode() == StringTraversals.Game.Data.GameMode.None);
			}
			
			private static void DisplayResults() {
				switch (StringTraversals.Game.Data.m_result) {
		...
		private static class Board {
			...
			private static StringTraversals.Game.Board ActiveBoard;
			
			public static StringTraversals.Game.Board ActiveBoard() { return ActiveBoard; }
			
			
			private static List<StringTraversals.Game.Board> GameBoards = new ArrayList<>();
			
			public static List<StringTraversals.Game.Board> GameBoards() { return GameBoards; }
			...
			public void PlayBoardRemoveIndex(String p_str) {
				int[] _index = StringTraversals.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
			...
			public void PlayBoardSwapIndex(String p_str, int p_x, int p_y) {
				int[] _index = StringTraversals.Utils.ArrayU.Array2DFind(m_playBoard, p_str);
				if (_index[0] > -1 && _index[1] > -1) StringTraversals.Utils.ArrayU.Array2DSwap(m_playBoard, _index[0], _index[1], p_x, p_y);
			}
			...
			public void RenderBoard(boolean p_updateBoard) {
				...
				for (String[] _arr : m_displayBoard)
					System.out.println(StringTraversals.Utils.ArrayU.ArrayToString(_arr, StringTraversals.Utils.ArrayU.LongestStringIn2DArray(m_displayBoard)));
			}
		...
		public static void GetUserFileData() {
			...
			while (!m_beenRead) {
				try {
					...
					// Gets user file name
					m_fileName = StringTraversals.User.UserString();
					System.out.println();
					...
					try {
						...
						m_beenRead = true;
						_scanner.close();
						if (StringTraversals.Utils.StringU.isStringNumber(m_fileData.get(0))) {
	...
	private static class User {
		...
		public StringTraversals.User WaitForValid(String [] p_ignores) {
			return this;
		}
		...
		public static float UserNum() {
			...
			if (StringTraversals.Utils.StringU.isStringNumber(m_userInput)) return Integer.parseInt(m_userInput);
			return -1;
		}
	...
	protected static class Utils {
		...
		private static class ArrayU {
			...
			public static <t_Any> String ArrayToString(t_Any[] p_arr, int p_spacing) {
				...
				return StringTraversals.Utils.StringU.SpaceEvenly(_result.substring(0, _result.length() - 1), " ", p_spacing);
			}
		...
		private static class StringU {
			...
			public static String SpaceEvenly(String p_str, String p_split, int p_spacing) {
				...
				if (p_spacing < 0) p_spacing = StringTraversals.Utils.ArrayU.LongestStringInArray(_items.toArray(new String[0]));
